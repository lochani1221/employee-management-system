...
const DepartmentByIdPage = () => {
  const { id } = useParams(); // Department ID from the route
  ...
  const fetchEmployeesByDeptId = () => {
    axios
      .get(`http://localhost:8080/api/V1/employees/department/id/${id}`)
  ...
  const handleDelete = (empId) => {
    if (window.confirm('Are you sure you want to delete this employee?')) {
      EmployeeService.deleteEmployee(empId).then(() => fetchEmployeesByDeptId());
    }
  };
  ...
  const handleUpdate = () => {
    EmployeeService.updateEmployee(selectedEmployee.id, selectedEmployee).then(() => {
    ...
    <div className="container mt-5">
      <div className="card shadow-lg">
        <div className="card-header bg-primary text-white">
          <h3 className="text-center mb-0">{departmentNames[id] || 'Department'} Employees</h3>
        </div>
        <div className="card-body">
          {employees.length > 0 ? (
              ...
              <table className="table table-bordered table-hover">
                <thead className="table-light text-center">
                  <tr>
                    ...
                    <th>Address</th><th>Gender</th><th>Status</th><th>Date Appointed</th><th>Actions</th>
                  </tr>
                </thead>
                <tbody>
                  {employees.map((emp) => (
            ...
            <p className="text-center text-muted">No employees found in this department.</p>
          )}
        </div>
      </div>
